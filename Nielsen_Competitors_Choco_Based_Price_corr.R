...
Nielsen_data <- fread("Raw Data/(BARS)MERGED 2017-2020 NIELSEN BARS DATA LONG FORMAT.csv")
...
#remove rolled up data and remove sku without weight(~4)
...
Nielsen_data1 <- Nielsen_data[,c("Manufacturer","Brand","SKU","PackSize")]
...
weight_delta=0.15
PPG_list=unique(na.omit(Nielsen_data1$PPG))
...
for(i in PPG_list)
{
  ...
  {
    idx=which(other_ppg_data$Competitor_Weight_in_range==0) #0.15
...
#in case duplicate
a=unique(tmp_competitor[tmp_competitor$Competitor_Weight_in_range==1,c("PPG","Competitor_SKU")])
#tmp_competitor=tmp_competitor[!duplicated(tmp_competitor[,c("Competitor_SKU")])]

#write.xlsx(SKU_competitor,"test.xlsx")

SKU_competitor <- SKU_competitor[!duplicated(SKU_competitor[,c("PPG","Competitor_SKU")])]
...
Nielsen_data_raw <- Nielsen_data_raw[Nielsen_data_raw$SKU %in% SKU_competitor$Competitor_SKU, ]
SKU_competitor_price=left_join(SKU_competitor,Nielsen_data_raw[,c("Date","Week","Year","Manufacturer","Brand","SKU","Nielsen_Sales","Nielsen_Units","Price_per_Unit")],by = c('Competitor_SKU'='SKU'))
#inf
...
setDT(SKU_competitor_price)
SKU_competitor_price <- left_join(SKU_competitor3[,c('PPG','Competitor_SKU')],
                              Nielsen_data_raw[,c('SKU','Date','Nielsen_Sales','Nielsen_Units','Price_per_Unit')],by=c('Competitor_SKU'='SKU'))
